...
public class HBaseFeatureRowMapper implements RowMapper<HBaseFeature> {
    ...
    public HBaseFeature mapRow(Result result, int rowNum) throws Exception {
        ...
        feature.setIsVip(Integer.parseInt(Bytes.toString(result.getValue(COLUMNFAMILY, ISVIP))));
        ...
        feature.setCountLikeGivingLatest7Days(Double.parseDouble(Bytes.toString(result.getValue(COLUMNFAMILY, LIKE_GIVE))));
        feature.setCountLikeReceivedLatest7Days(Double.parseDouble(Bytes.toString(result.getValue(COLUMNFAMILY, LIKE_RECEIVE))));
        feature.setCountMatchLatest7Days(Double.parseDouble(Bytes.toString(result.getValue(COLUMNFAMILY, MATCHES))));
        feature.setRateGivingLikeOverImpressionLatest7Days(Double.parseDouble(Bytes.toString(result.getValue(COLUMNFAMILY, RATE_LIKE_GIVE))));
        feature.setRateReceivedLikeOverImpressionLatest7Days(Double.parseDouble(Bytes.toString(result.getValue(COLUMNFAMILY, RATE_LIKE_RECEIVE))));
        feature.setRateMatchOverImpressionLatest7Days(Double.parseDouble(Bytes.toString(result.getValue(COLUMNFAMILY, RATE_MATCHE))));
        feature.setMlcType(Integer.parseInt(Bytes.toString(result.getValue(COLUMNFAMILY, MLC_TYPE))));
